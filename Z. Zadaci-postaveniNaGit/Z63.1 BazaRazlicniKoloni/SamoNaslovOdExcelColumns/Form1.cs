...
namespace SamoNaslovOdExcelColumns
{
    public partial class Form1 : Form
    {
        ...
        public void ReadData(SQLiteConnection conn, string imeNaTabela)
        {
        ...
        public void InsertRow(SQLiteConnection conn, List<string> listView, string[] listValues, string imeNaTabela)
        {
            ...
            try
            {
                ...
                this.openFileDialog1.FileName = "*.xls"; if (this.openFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (tbImeNaTabela.Text != "")
                    {
                        ...
                        {
                            if (tbImeNaTabela.Text == "PredmetiCelosni")
                            {
                                krajnaKolona = "M";
                            }
                            else if (tbImeNaTabela.Text == "PredmetiNecelosni")
                            {
                                krajnaKolona = "I";
                            }
                            Excel.Range range = worksheet.get_Range("A" + i.ToString(), krajnaKolona + i.ToString());
                            System.Array myvalues = (System.Array)range.Cells.Value;
                            string[] strArray = ConvertToStringArray(myvalues);
                            ListViewItem itm;
                            itm = new ListViewItem(strArray);
                            listView1.Items.Add(itm);
                            InsertRow(sqlite_conn, listView, strArray, tbImeNaTabela.Text);
                        }
                    }
