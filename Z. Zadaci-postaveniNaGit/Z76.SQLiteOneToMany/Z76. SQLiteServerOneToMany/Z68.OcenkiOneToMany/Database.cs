    ...
    internal class Database
    {
        public class SQLiteManager
        {
            ...
            public void CreateTables()
            {
             ...
             "CREATE TABLE IF NOT EXISTS Ocenki (ID INTEGER PRIMARY KEY AUTOINCREMENT, Клас TEXT, Смер TEXT, Тромесечие TEXT,Учебна_Година TEXT, Македонски_јазик INTEGER,Математика INTEGER,Англиски_јазик INTEGER,Програмирање INTEGER,Физика INTEGER,Автоматика INTEGER,Објектно_ориентирано_програмирање INTEGER,Музика INTEGER,Изборен INTEGER,Практична_настава INTEGER,Статистика INTEGER, UceniciEMB INTEGER, FOREIGN KEY(UceniciEMB) REFERENCES Ucenici(ID))", connection))
                {
                    cmd.ExecuteNonQuery();
                }
            ...
            public SQLiteDataReader SelectFromToExcel(string imeNaTabela)
            {
                OpenConnection();
                SQLiteDataReader reader;
                using (SQLiteCommand cmd = new SQLiteCommand(
